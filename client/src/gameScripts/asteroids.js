...
export default class Asteroid {
    ...
    increaseScoreBySize(size, service){
        switch(size) {
            case 2:
                service.player.score += 300;
                this.fx.score.innerHTML = service.player.score;
                return;
            case 3:
                service.player.score += 500;
                this.fx.score.innerHTML = service.player.score;
                return;
            default:
                service.player.score += 100;
                this.fx.score.innerHTML = service.player.score;
                return;
        }
    }
        ...
        if(this.active) {
            phasers.forEach(p => {
                if(this.hasCollidedWithEntity(p)){
                    ...
                    this.increaseScoreBySize(this.size, service);
